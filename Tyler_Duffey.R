...
tyler.pre <- pitch %>% filter(pitcher == 608648) %>%
...
table(tyler$simple_pitch_type)

# Get rid of unknowns
tyler <- tyler %>% filter(simple_pitch_type != "UN" & simple_pitch_type != "PO") %>%
  mutate(simple_pitch_type = as.character(simple_pitch_type))

# Break
ggplot(data = tyler, aes(pfx_x, pfx_z, color = simple_pitch_type)) + geom_point()

# Velocity
ggplot(data = tyler, aes(start_speed)) + facet_grid(simple_pitch_type~.) + geom_histogram()

ggplot(data = tyler, aes(start_speed, fill = simple_pitch_type)) + 
...
ggplot(data = filter(tyler, pitch_result %in% c("Ball", "Ball In Dirt", "Called Strike")), 
...
ggplot(data = tyler, aes(px, pz, color = simple_pitch_type)) + geom_point()

tyler <- tyler %>% mutate(k = simple_event == "K" & last)

ggplot(data = filter(tyler, count == "0-2"), aes(px, pz, color = simple_pitch_type, shape = k)) + 
  geom_point(size = 2) +
  geom_polygon(data = strike.zone, aes(x = x, y = y, color = NA, shape = NA), fill = NA, color = "black") +
  coord_fixed()


ggplot(data = tyler, aes(px, pz, color = simple_pitch_type, shape = k)) + 
  geom_point() +
  geom_polygon(data = strike.zone, aes(x = x, y = y, color = NA), fill = NA, color = "black") +
  coord_fixed()
...
tables <- tyler %>% group_by(count, balls, strikes, b_hand) %>%
