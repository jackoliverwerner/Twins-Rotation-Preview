...
ggplot(data = kyle, aes(px, pz, color = simple_pitch_type)) + geom_point()

kyle <- kyle %>% mutate(k = simple_event == "K" & last)

ggplot(data = filter(kyle, count == "0-2"), aes(px, pz, color = simple_pitch_type, shape = k)) + 
  geom_point(size = 2) +
  geom_polygon(data = strike.zone, aes(x = x, y = y, color = NA, shape = NA), fill = NA, color = "black") +
  coord_fixed()


ggplot(data = kyle, aes(px, pz, color = simple_pitch_type, shape = k)) + 
  geom_point() +
  geom_polygon(data = strike.zone, aes(x = x, y = y, color = NA), fill = NA, color = "black") +
  coord_fixed()
